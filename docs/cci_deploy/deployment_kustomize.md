...
The deployment step applies the Kustomize-rendered manifests to the cluster configured in the `KUBECONFIG_DATA` environment variable.
...
- run:
    ...
    command: |
      circleci run release plan \
        ...
        --component-name=cci-deploy-kustomize-demo \
        --target-version=$VERSION \
        kustomize-deploy-release
```
...
release:
  type: release
  plan_name: kustomize-deploy-release
```

...
The `install-chart` step proceeds to install the Argo Rollout chart into the kubernetes cluster configured in the `KUBECONFIG_DATA` environment variable.
...
- run:
    ...
    command: |
      circleci run release plan \
        ...
        --component-name=helm-cci-rollout-demo \
        --target-version=$VERSION \
        --release-strategy=progressive \
        helm-rollout-release
```
...
release:
  type: release
  plan_name: helm-rollout-release
```
